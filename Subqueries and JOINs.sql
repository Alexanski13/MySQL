SELECT d.manager_id, CONCAT_WS(' ', e.first_name, e.last_name) AS full_name, d.department_id, d.name AS department_name FROM departments AS d
JOIN employees AS e
...
WHERE e.salary > (SELECT AVG(e1.salary) FROM employees AS e1);
