package gamestore.domain.dtos.game;
...
public class GameDto {
    ...
    public void setTitle(String title) {
        if (title != null && Character.isLowerCase(title.charAt(0)) || title.length() <= 3 || title.length() >= 100) {
    ...
    public void setImageUrl(String imageUrl) {
        if (imageUrl != null && !imageUrl.startsWith("http://") || !imageUrl.startsWith("https://")) {
            throw new IllegalArgumentException("Link should begin with http/s ...");
        }
    ...
    public void setSize(float size) {
        if (size <= 0.0) {
            throw new IllegalArgumentException("Price or size should be a positive number.");
        }
        this.size = size;
    }
