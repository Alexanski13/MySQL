...
genre_id INT,
movie_id INT,
KEY pk_movies_actors (genre_id, movie_id),
...
SELECT m.id, m.title, mai.runtime, mai. budget, mai.release_date FROM movies AS m
...
ELSE 'excellent'
END
) AS rating, IF (mai.has_subtitles = 1, 'english', '-') AS subtitles, mai.budget FROM movies AS m
JOIN movies_additional_info AS mai ON m.movie_info_id = mai.id
